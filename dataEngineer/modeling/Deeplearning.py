...
class BertTextClassifier:
    ...
    def __init__(
        self, model_name="bert-base-uncased", model_dir="./bert_model", max_length=128
    ):
        """
        ...
        self._label_encoder_path = os.path.join(
            self.model_dir, "label_encoder.classes_.npy"
        )
    ...
    def save(self):
        ...
        if self.model:
            # --- FIX: Explicitly save in the TensorFlow H5 format ---
            self.model.save_pretrained(self.model_dir, save_format="h5")
            ...
            with open(self._config_path, "w") as f:
                json.dump(config, f)
            print(
                f"Model, tokenizer, and configuration saved successfully to {self.model_dir}/"
            )
        else:
    ...
    def load(self):
        ...
        if not os.path.exists(self._config_path):
            print(
                f"Error: Model configuration not found in directory {self.model_dir}/"
            )
            return False
        try:
            ...
            self.label_encoder.classes_ = np.load(
                self._label_encoder_path, allow_pickle=True
            )
            self.tokenizer = BertTokenizer.from_pretrained(self.model_dir)
            # This should now work correctly because the model was saved in the right format
    ...
    def train(
        ...
        if os.path.exists(self._config_path):
            ...
            if not new_labels.issubset(known_labels):
                ...
                raise ValueError(
                    f"New data contains labels the model was not trained on: {unknown}. Incremental training requires the same set of labels."
                )
